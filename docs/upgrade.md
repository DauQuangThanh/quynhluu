...
> You have Quynhluu installed and want to upgrade to the latest version to get new features, bug fixes, or updated slash commands. This guide covers both upgrading the CLI tool and updating your project files.
...
| ---------------- | --------- |-------------|
| **CLI Tool Only** | `uv tool install quynhluu-cli --force --from git+https://github.com/dauquangthanh/quynhluu.git` | Get latest CLI features without touching project files |
| **Project Files** | `quynhluu init --here --force --ai <your-agent>` | Update slash commands, templates, and scripts in your project |
...
The CLI tool (`quynhluu`) is separate from your project files. Upgrade it to get the latest features and bug fixes.
...
```bash
uvx --from git+https://github.com/dauquangthanh/quynhluu.git quynhluu init --here --ai copilot
```
...
When Quynhluu releases new features (like new slash commands or updated templates), you need to refresh your project's Quynhluu files.

### What gets updated?

Running `quynhluu init --here --force` will update:

- ✅ **Slash command files** (`.claude/commands/`, `.github/prompts/`, etc.)
- ✅ **Script files** (`.quynhluu/scripts/`)
- ✅ **Template files** (`.quynhluu/templates/`)
...
```bash
quynhluu init --here --force --ai <your-agent>
```
...
**Known issue:** `quynhluu init --here --force` currently overwrites `docs/ground-rules.md` with the default template, erasing any customizations you made.
...
If you customized any templates in `.quynhluu/templates/`, the upgrade will overwrite them. Back them up first:

```bash
# Back up custom templates
cp -r .quynhluu/templates .quynhluu/templates-backup
...
rm quynhluu.specify-old.md
rm quynhluu.design-v1.md
```
...
uv tool install quynhluu-cli --force --from git+https://github.com/dauquangthanh/hanoi-quynhluu.git
...
cp -r .quynhluu/templates /tmp/templates-backup
...
rm quynhluu.old-command-name.md
...
# Run upgrade
quynhluu init --here --force --ai copilot --no-git
...
```bash
quynhluu init my-project --ai copilot --no-git
```
...
This warning appears when you run `quynhluu init --here` (or `quynhluu init .`) in a directory that already has files. It's telling you:
...
- Templates in `.quynhluu/templates/`
...
```bash
uv tool uninstall quynhluu-cli
uv tool install quynhluu-cli --from git+https://github.com/dauquangthanh/hanoi-quynhluu.git
```

### "Do I need to run quynhluu every time I open my project?"

**Short answer:** No, you only run `quynhluu init` once per project (or when upgrading).
...
Once you've run `quynhluu init`, the slash commands (like `/quynhluu.specify`, `/quynhluu.design`, etc.) are **permanently installed** in your project's agent folder (`.claude/`, `.github/prompts/`, etc.). Your AI assistant reads these command files directly—no need to run `quynhluu` again.
...
3. **Check you're in the correct directory** where you ran `quynhluu init`

4. **For some agents**, you may need to reload the workspace or clear cache

**Related issue:** If Copilot can't open local files or uses PowerShell commands unexpectedly, this is typically an IDE context issue, not related to `quynhluu`. Try:
...
- **Test new slash commands:** Run `/quynhluu.set-ground-rules` or another command to verify everything works
- **Review release notes:** Check [GitHub Releases](https://github.com/dauquangthanh/hanoi-quynhluu/releases) for new features and breaking changes
