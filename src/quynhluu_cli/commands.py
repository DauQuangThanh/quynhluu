...
def init(
    ...
    upgrade: bool = typer.Option(False, "--upgrade", help="Upgrade existing Quynhluu project by replacing .quynhluu and agent folders with latest templates"),
    ...
    """
    Initialize a new Quynhluu project from the latest template.
    ...
    Examples:
        ...
        quynhluu init my-project --ai claude,cursor-agent --script sh
        ...
        quynhluu init --upgrade
        quynhluu init --upgrade --ai claude
        quynhluu init --here --upgrade

        # Use local templates for development
        quynhluu init demo --local-templates --ai claude
        quynhluu init demo --local-templates --template-path /path/to/hanoi-quynhluu

        # With environment variables
        export RAINBOW_USE_LOCAL_TEMPLATES=1
        export RAINBOW_TEMPLATE_PATH=/path/to/hanoi-quynhluu
        quynhluu init demo --ai copilot
    """
    ...
    if here:
        ...
        if project_path.exists() and not upgrade:
            # Check if Quynhluu is already installed in this directory
            quynhluu_dir = project_path / ".quynhluu"
            if quynhluu_dir.exists():
                # Quynhluu is installed, suggest upgrade
                error_panel = Panel(
                    f"Directory '[cyan]{project_name}[/cyan]' already exists with Quynhluu installed\n\n"
                    "To upgrade the existing Quynhluu installation, use:\n"
                    f"  [cyan]quynhluu init {project_name} --upgrade[/cyan]\n\n"
                    "Or choose a different project name.",
                    title="[yellow]Quynhluu Already Installed[/yellow]",
                    border_style="yellow",
                    padding=(1, 2)
                )
                console.print()
                ...
                # Directory exists but no Quynhluu installation
                error_panel = Panel(
                    f"Directory '[cyan]{project_name}[/cyan]' already exists\n\n"
                    "Quynhluu is not installed in this directory. You can:\n"
                    "  • Use [cyan]--force[/cyan] to merge Quynhluu files into the existing directory\n"
                    f"  • Use [cyan]quynhluu init . --force[/cyan] from within the directory\n"
                ...
                if not force and sys.stdin.isatty():
                    response = typer.confirm("\nDo you want to proceed and merge Quynhluu files into this directory?")
                    if response:
    ...
    if upgrade:
        quynhluu_dir = project_path / ".quynhluu"
        if not quynhluu_dir.exists():
            error_panel = Panel(
                f"No Quynhluu project found in '[cyan]{project_path}[/cyan]'\n"
                "The .quynhluu folder does not exist. Cannot upgrade.\n\n"
                "Use 'quynhluu init' without --upgrade to initialize a new project.",
        ...
        upgrade_lines = [
            ...
            "  • .quynhluu/ folder (scripts, templates, memory)",
        ]
    ...
    setup_lines = [
        f"[cyan]Quynhluu Project {'Upgrade' if is_upgrade_mode else 'Setup'}[/cyan]",
        "",
    ...
    tracker = StepTracker("Upgrade Quynhluu Project" if is_upgrade_mode else "Initialize Quynhluu Project")
    ...
    with Live(tracker.render(), console=console, refresh_per_second=8, transient=True) as live:
        ...
        try:
            ...
            if is_upgrade_mode:
                ...
                # Backup .quynhluu folder
                quynhluu_dir = project_path / ".quynhluu"
                if quynhluu_dir.exists():
                    backup_quynhluu = project_path / f".quynhluu.backup.{timestamp}"
                    shutil.copytree(quynhluu_dir, backup_quynhluu)
                    backup_paths[".quynhluu"] = backup_quynhluu
            ...
            # Only copy shared .quynhluu folder for the first agent to avoid redundancy
    ...
    steps_lines.append("   • [cyan]/quynhluu.set-ground-rules[/] - Establish project principles (ground rules)")
    steps_lines.append("   • [cyan]/quynhluu.specify[/] - Create feature specification")
    steps_lines.append("   • [cyan]/quynhluu.architect[/] [dim](optional, product-level)[/dim] - Define system architecture")
    steps_lines.append("   • [cyan]/quynhluu.standardize[/] [dim](optional, product-level)[/dim] - Establish coding standards")
    steps_lines.append("   • [cyan]/quynhluu.design[/] - Create technical implementation plan")
    steps_lines.append("   • [cyan]/quynhluu.taskify[/] - Generate actionable tasks")
    steps_lines.append("   • [cyan]/quynhluu.implement[/] - Execute implementation")
    ...
    enhancement_lines = [
        ...
        "",
        f"○ [cyan]/quynhluu.clarify[/] - Ask structured questions before planning (run before [cyan]/quynhluu.design[/])",
        f"○ [cyan]/quynhluu.analyze[/] - Cross-artifact consistency report (after [cyan]/quynhluu.taskify[/], before [cyan]/quynhluu.implement[/])",
        f"○ [cyan]/quynhluu.checklist[/] - Generate quality checklists (after [cyan]/quynhluu.design[/])",
        f"○ [cyan]/quynhluu.design-e2e-test[/] - Design end-to-end test specs (product-level)",
        f"○ [cyan]/quynhluu.perform-e2e-test[/] - Execute E2E tests and generate reports"
    ]
...
def check():
    ...
    console.print("\n[bold green]Quynhluu CLI is ready to use![/bold green]")
...
def version():
    ...
    cli_version = "unknown"
    try:
        cli_version = importlib.metadata.version("quynhluu-cli")
    except Exception:
    ...
    panel = Panel(
        info_table,
        title="[bold cyan]Quynhluu CLI Information[/bold cyan]",
        border_style="cyan",
        padding=(1, 2)
    )
