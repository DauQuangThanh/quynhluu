"""Template operations for Quynhluu CLI."""
...
def copy_local_template(
    ...
    Args:
        is_first_agent: If True, copies shared .quynhluu folder. If False, skips it.
    ...
    # Only copy shared .quynhluu folder for the first AI assistant
    # This prevents redundant copying when multiple AI assistants are selected
    if is_first_agent:
        # Create .quynhluu directory structure
        quynhluu_dir = project_path / ".quynhluu"
        quynhluu_dir.mkdir(exist_ok=True)

        # Copy memory
        if memory_dir.exists():
            dest_memory = quynhluu_dir / "memory"
        ...
        if scripts_dir.exists():
            dest_scripts = quynhluu_dir / "scripts"
        ...
        # Copy templates to .quynhluu/templates with subdirectories
        dest_templates = quynhluu_dir / "templates"
        dest_templates.mkdir(exist_ok=True)

        # Copy from agent-commands/templates-for-commands to .quynhluu/templates/templates-for-commands
    ...
    # prefixed with "quynhluu." and use agent-specific extensions, preventing conflicts.
    ...
    if commands_dir.exists():
        for cmd_file in commands_dir.glob("*.md"):
            ...
            output_file = agent_path / f"quynhluu.{cmd_file.stem}{agent_ext}"
...
def download_and_extract_template(
    ...
    *,
    ...
    Args:
        is_first_agent: If True, copies shared .quynhluu folder. If False, skips it to avoid redundancy.
    """
    ...
    if local_templates:
        ...
        if template_path:
            ...
            # Default to repo root (assume we're in src/quynhluu_cli)
            ...
            if is_current_dir:
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    for item in source_dir.iterdir():
                        # Skip .quynhluu folder for non-first agents to avoid redundancy
                        if not is_first_agent and item.name == ".quynhluu":
                            continue
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    # Copy items from temp to project_path, skipping .quynhluu for non-first agents
                    for item in source_dir.iterdir():
                        # Skip .quynhluu folder for non-first agents to avoid redundancy
                        if not is_first_agent and item.name == ".quynhluu":
                            continue
